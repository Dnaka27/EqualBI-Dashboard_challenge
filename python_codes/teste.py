...
if selected == "Produtos":
    ...
    with st.container():   
        ...
        def render_by_role(role):
            ...
            def write_plotBar_column_byRole():
...
if selected == "Filiais":
    with st.container():
        ...
        df_saleBranch_groupBranch = df_saleBranch.groupby(["filial_venda"])["valor_monetario_total"].sum().reset_index()
        
        df_saleBranch_groupBranch_date = df_saleBranch.groupby(["filial_venda", "mes", "ano"])["valor_monetario_total"].sum().reset_index()
